...
This page documents how we selected alters for this project. We use non-public/non-anonymized data of the 'Sports and Friendships' study [Franken et al., -@data2023].
...
```{r eval=F}
data1 <- read.csv("./rawdata/results-survey493144.csv") #wave1 
data2a <- read.csv("./rawdata/results-survey952354a.csv") #wave2a
data2b <- read.csv("./rawdata/results-survey952354b.csv") #wave2b
```
...
for (i in unique(df$ego)) {  
    ...
    alter <- unlist(alterL[[i]][j,3:6], use.names = F) # get name(s) of alter j
...
for (i in unique(df$ego)) {
    for (j in 1:nrow(df[which(df$ego==i),])) { 
      ...
      df$kin[which(df$ego==i & df$alterid==j)] <- alterL[[i]][j,4]
...
for ( i in 1:length(alterL)) {
  alterL[[i]] <- data.frame(
    ...
    kin=NA,
    name1=NA, name2=NA, name3=NA, name4=NA,
    cdn=NA,study=NA,friend=NA,csn=NA,sports=NA
  )
}
...
for (i in unique(df$ego)) {
    for (j in 1:nrow(df[which(df$ego==i),])) {
      ...
      df$alter_age[which(df$ego==i & df$alterid==j)] <- alterL[[i]][j,3]
      df$alter_gender[which(df$ego==i & df$alterid==j)] <- alterL[[i]][j,2]
      df$sports[which(df$ego==i & df$alterid==j)] <- alterL[[i]][j,13]
    }
}
df_created <-df
...
#names(df_maintained)
df_maintained <- df_maintained[,-c(15,7:10)]
